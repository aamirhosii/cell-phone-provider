...
def process_event_history(log: dict[str, list[dict]],
    ...
    for item in log['events']:
        d = datetime.datetime.strptime(item['time'], "%Y-%m-%d %H:%M:%S")
        if item['type'] == 'call':

            call = Call(item['src_number'],
                        item['dst_number'],
                        d,
                        item['duration'],
                        item['src_loc'],
            ...
            x = find_customer_by_number(call.src_number,
                                        customer_list)
            y = find_customer_by_number(call.dst_number,
                                        customer_list)
            Customer.make_call(x, call)
            Customer.receive_call(y, call)

        if d.month != billing_month:
            billing_month = d.month
            new_month(customer_list, d.month, d.year)
